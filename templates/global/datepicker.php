...
<form name="hotelier_datepicker" method="post" id="hotelier-datepicker" class="datepicker-form" action="<?php echo esc_url( $datepicker_form_url ); ?>" enctype="multipart/form-data">
	...
	<span class="datepicker-input-select-wrapper">
		<?php do_action( 'hotelier_uncode_datepicker_before_input', $shortcode_atts ); ?>
	...
	<?php
	$datepicker_button_class = htl_uncode_get_button_classes( $shortcode_atts, 'datepicker_button' );
	?>

	<?php echo apply_filters( 'hotelier_datepicker_button_html', '<input type="submit" class="button button--datepicker ' . esc_attr( implode( ' ', $datepicker_button_class ) ) . '" name="hotelier_datepicker_button" id="datepicker-button" value="' . esc_attr__( 'Check availability', 'wp-hotelier' ) . '" />' ); ?>
</form>
