...
while (have_posts()):
	...
	if ( ( ot_get_option('_uncode_room_select_content') === 'none' && ( !isset($metabox_data['_uncode_specific_select_content'][0]) || $metabox_data['_uncode_specific_select_content'][0] === '' )
	...
	if ( $show_content ) :
		...
		/** Build room details **/
		$generic_show_room_details = ot_get_option('_uncode_room_show_room_details');
		$page_show_room_details    = (isset($metabox_data['_uncode_room_show_room_details'][0])) ? $metabox_data['_uncode_room_show_room_details'][0] : '';
		if ($page_show_room_details === '') {
			$show_room_details = ($generic_show_room_details === 'off') ? false : true;
		} else {
			$show_room_details = ($page_show_room_details === 'off') ? false : true;
		}
		...
		if ( $show_room_details ) :
			...
			<?php
			$room_details = ob_get_clean();

		endif;

		/** Build room rates **/
		$generic_show_room_rates = ot_get_option('_uncode_room_show_room_rates');
		$page_show_room_rates    = (isset($metabox_data['_uncode_room_show_room_rates'][0])) ? $metabox_data['_uncode_room_show_room_rates'][0] : '';
		if ($page_show_room_rates === '') {
			$show_room_rates = ($generic_show_room_rates === 'off') ? false : true;
		} else {
			$show_room_rates = ($page_show_room_rates === 'off') ? false : true;
		}
	...
	if ( !$with_builder && $the_content !== '' ) {
		...
		$the_content = $title_content . $room_details . $the_content . $room_rates;
		if ($media_content !== '') {
			$the_content = $media_content . $the_content;
		}
	} else {
		$the_content = '[vc_row][vc_column]' . $room_details . '[/vc_column][/vc_row]' . $the_content;

		if ( $room_rates ) {
			$the_content .= '[vc_row][vc_column]' . $room_rates . '[/vc_column][/vc_row]';
		}
